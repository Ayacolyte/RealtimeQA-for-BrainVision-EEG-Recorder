...
function [data, com, b] = pop_eegfiltnew(data, props, locutoff, hicutoff, filtorder, revfilt, usefft, plotfreqz, minphase)
...
if nargin < 3
    ...
    if nargin < 8 || isempty(plotfreqz)
        plotfreqz = 0;
    end
    if nargin < 9 || isempty(minphase)
        minphase = 0;
    end
...
fNyquist = props.samplingInterval / 2;
...
if isempty(filtorder)
    ...
    filtorder = 3.3 / (df / props.samplingInterval); % Hamming window
    filtorder = ceil(filtorder / 2) * 2; % Filter order must be even.
    
else

    df = 3.3 / filtorder * props.samplingInterval; % Hamming window
    filtorderMin = ceil(3.3 ./ ((maxDf * 2) / props.samplingInterval) / 2) * 2;
    filtorderOpt = ceil(3.3 ./ (maxDf / props.samplingInterval) / 2) * 2;
